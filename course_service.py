...
def get_all_subjects_from_db(db: Session, current_user: User = Depends(get_current_user)):
    ...
    if len(subjects_lst) == 0:
        raise HTTPException(status_code=404, detail="Courses  are not found")
...
def create_new_course_in_db(*,db: Session,data: CreateNewCourse,current_user: User = Depends(get_current_user)):
    ...
    if (
        ...
        raise HTTPException(status_code=400, detail="Course name is already exists")
...
def registration_in_db(*,data: RegistrationData,db: Session,current_user: User = Depends(get_current_user)):
    ...
    if student_in_registration:
        raise HTTPException(status_code=400, detail="Student is alredy registrared")
        ...
        return {"Message": "Student is registrated succesfully"}


def get_course_info_from_db(db: Session, current_user: User = Depends(get_current_user)):
    current_user_in_db = (db.query(User).filter(User.username == current_user["sub"]).first())

    if current_user_in_db.role == "admin":
        raise HTTPException(status_code=401, detail="Only lecturers can get info about courses")
...
def delete_course_from_db(*, id: int, db: Session, current_user: User = Depends(get_current_user)):
    current_user_in_db = (db.query(User).filter(User.username == current_user["sub"]).first())
