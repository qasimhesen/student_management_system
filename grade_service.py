...
def get_grade_by_id_from_db(*, student_id: int, db: Session, current_user: User = Depends(get_current_user)):
    ...
    student = (db.query(Student).filter(Student.id == student_id, Student.is_deleted == False).all())
...
def get_grade_by_course_id_from_db(*, course_id: int, db: Session, current_user: User = Depends(get_current_user)):
    current_user_in_db = (db.query(User).filter(User.username == current_user["sub"]).first())

    course_in_db = (db.query(Course).filter(Course.id == course_id, Course.is_deleted == False).first())
